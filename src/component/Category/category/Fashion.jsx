...
const Fashion = () => {
  let {fashion}=useContext(providerContext)
  
  return (
    <div >
      <div className='text-center'>
        <h1 className='pt-10 text-3xl max-sm:text-xl  font-semibold'>Category Fashion </h1>
      </div>
      <div className='pt-5 lg:px-15 max-sm:px-6 md:px-10  flex flex-wrap gap-4  items-center mb-4 justify-center'>
        {fashion.map((elem)=>{
          ...
          <div className='px-4 pt-3'>
            ...
            <button disabled={elem.sold}   className={`color w-full py-2 mb-2 ${elem.sold ? 'cursor-not-allowed':'cursor-pointer'} rounded text-white font-medium  `}>Add To Cart </button>
          </div>
        </div>
